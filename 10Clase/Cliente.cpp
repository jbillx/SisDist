...
int main()
{
   ...
   char ip[] = "127.0.0.1";
   unsigned int *respuesta; //apunta a un entero que puedo enviar a traves del socket
...
for (int i=0;i<noServidores;i++)}{
   unidad=num/noServidores;
   rango[i]= unidad*i;
   rango[noServidores]=num;
}
   


   PaqueteDatagrama paq((char *)num, 3 * sizeof(unsigned int), ip, puerto);
   ...
   respuesta = (unsigned int *)paquete1.obtieneDatos(); //obtiene a traves de un metodo los datos que le envio el servidor
